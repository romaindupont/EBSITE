...
const Articles = ({count, open, setCount,articleData,changeArticlesField,listArticles}) => {
  ...
  return (
    <div className={classNames("articles", {"articles--open":open})}>
      ...
      {slug==="all" ? (list.map((article)=>
        <Card key={article.id} count={count} setCount={setCount} {...article}/>)):(
          <Card key={currentCate.id} count={count} setCount={setCount} {...currentCate}/>
      )}
      </div>
